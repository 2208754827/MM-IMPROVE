#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""L1-Norm structured iterative pruning for RTDETRMM VIF models.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="L1-Norm iterative pruning for RTDETRMM VIF models")
    parser.add_argument("--weights", type=str, default=str(default_weights), help="Input .pt checkpoint")
    parser.add_argument("--output", type=str, default="prune_outputs_l1_only", help="Output .pt path or directory")
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_l1_r{ratio_str}_{timestamp}.pt"
    ...
    while True:
        ...
        try:
            ...
            if best_candidate is None:
                log(f"Round {round_idx}: no more safe L1 pruning candidates.")
                break

            raw_score, spec, idxs = best_candidate
            log(
                f"Round {round_idx}: prune {spec.name} "
                f"(layer={spec.layer_idx}, n_prune={len(idxs)}, l1={raw_score:.4e})"
            )
            try:
