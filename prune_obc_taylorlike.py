#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""OBC-TaylorLike structured iterative pruning for RTDETRMM RGB/IR fusion models.
...
- Uses torch-pruning MetaPruner/DepGraph with an OBC-style Jacobian criterion for root pruning.
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_obc_taylorlike_r{ratio_str}_{timestamp}.pt"
...
def sanitize_checkpoint_for_save(orig_ckpt: dict | None, model: nn.Module) -> dict:
    ...
    ckpt["prune_method"] = "obc_taylorlike"
    return ckpt
    ...
    while True:
        ...
        try:
            ...
            for spec in current_specs:
                ...
                try:
                    scores = obc_scores_for_conv_out_channels(spec.module)
