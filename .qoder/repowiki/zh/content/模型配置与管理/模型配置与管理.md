...
- [说明.md（RTMM r18）](file://ultralytics/cfg/models/rtmm/r18/说明.md)
- [说明.md（RTMM r50）](file://ultralytics/cfg/models/rtmm/r50/说明.md)
...
- [__init__.py（多模态模块）](file://ultralytics/nn/mm/__init__.py)
- [rtdetr-r50-mm-mid-aifi-dattention.yaml](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-dattention.yaml)
- [rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml)
</cite>

## 更新摘要
**所做更改**
- 新增RTDETR多模态配置集合的详细说明，包括基于ResNet-18和ResNet-50的配置目录结构
- 扩展融合架构配置内容，重点介绍基于动态注意力机制的高级融合模型配置
- 新增基于TransformerEncoderLayer_DAttention和ASSA_SEFN_Mona_DyT等动态注意力融合技术
- 更新RTDETRMM模型变体的配置示例，展示多样化的融合策略
- 完善RTDETRMM与传统RT-DETR的架构分离说明
- 新增多模态路由器和配置解析器的核心组件说明
...
**更新** 新增RTDETRMM模型配置集合的全面介绍，包括基于不同主干网络的配置变体和先进的融合架构。特别关注基于动态注意力机制的高级融合模型配置，支持更复杂的跨模态特征整合策略。
...
- 配置与模型
  - 模型配置目录：ultralytics/cfg/models
    ...
    - RTDETRMM：ultralytics/cfg/models/rtmm/{backbone}
      - ResNet-18系列：ultralytics/cfg/models/rtmm/r18
      - ResNet-50系列：ultralytics/cfg/models/rtmm/r50
...
- 多模态核心模块
  - 路由器模块：ultralytics/nn/mm
  - RTDETRMM模型：ultralytics/models/rtdetrmm
...
C --> C1["ResNet-18系列<br/>ultralytics/cfg/models/rtmm/r18"]
C --> C2["ResNet-50系列<br/>ultralytics/cfg/models/rtmm/r50"]
...
I["多模态路由器<br/>ultralytics/nn/mm"] --> F
J["RTDETRMM模型<br/>ultralytics/models/rtdetrmm"] --> F
K["动态注意力融合示例<br/>TransformerEncoderLayer_DAttention"] --> C2
L["高级融合技术<br/>ASSA_SEFN_Mona_DyT"] --> C2
```
...
- [说明.md（RTMM r18）:1-8](file://ultralytics/cfg/models/rtmm/r18/说明.md#L1-L8)
- [说明.md（RTMM r50）:1-8](file://ultralytics/cfg/models/rtmm/r50/说明.md#L1-L8)
...
- [__init__.py（多模态模块）:1-78](file://ultralytics/nn/mm/__init__.py#L1-L78)
- [rtdetr-r50-mm-mid-aifi-dattention.yaml:1-46](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-dattention.yaml#L1-L46)
- [rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml:1-46](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml#L1-L46)
...
- **新增** RTDETRMM配置集合：基于不同主干网络（ResNet-18/50）的多样化配置变体，包含先进的融合架构如频域融合和动态注意力融合。
- **新增** 多模态路由器模块：提供MultiModalRouter、MultiModalConfigParser等核心组件，支持零拷贝张量路由和配置驱动的数据流。
...
- [__init__.py（多模态数据）:1-21](file://ultralytics/data/multimodal/__init__.py#L1-L21)
- [__init__.py（多模态引擎）:1-9](file://ultralytics/engine/multimodal/__init__.py#L1-L9)
- [model.py（RTDETRMM）:148-188](file://ultralytics/models/rtdetrmm/model.py#L148-L188)
- [__init__.py（多模态模块）:29-43](file://ultralytics/nn/mm/__init__.py#L29-L43)

## 架构总览
多模态配置采用"配置驱动 + 第五字段路由"的统一范式，通过MultiModalRouter接管不同模态的输入与特征流，确保YOLO与RT-DETR均可复用同一套书写规则。RTDETRMM作为独立模型家族，基于不同主干网络提供多样化的融合策略，包括早期融合、中期融合和频域融合等先进架构。新增的动态注意力机制进一步增强了跨模态特征整合能力。
...
END["RTDETRMM配置集合<br/>rtmm/{backbone}"]
END --> END1["ResNet-18系列<br/>rtdetr-r18-mm-*.yaml"]
END --> END2["ResNet-50系列<br/>rtdetr-r50-mm-*.yaml<br/>包含动态注意力融合"]
end
subgraph "模型层"
YMM["YOLOMM<br/>yolo11n-mm-early.yaml"]
RMM18["RTDETRMM(r18)<br/>基于ResNet-18的配置"]
RMM50["RTDETRMM(r50)<br/>基于ResNet-50的配置<br/>支持动态注意力机制"]
end
subgraph "多模态核心"
ROUTER["MultiModalRouter<br/>零拷贝路由"]
...
ENDENG["RTDETRMM引擎<br/>rtdetrmm/engine"]
end
CFG --> YMM
CFG --> RMM18
CFG --> RMM50
...
- **新增** 高级融合技术：频域融合（FreqFusion）、注意力融合、动态注意力融合等，提供更精细的特征交互机制。
...
- **新增** RTDETRMM高级配置：基于ResNet-18和ResNet-50的不同主干网络，提供多样化的融合策略和性能选择，包括动态注意力机制。
- **新增** 动态注意力融合：支持TransformerEncoderLayer_DAttention、ASSA_SEFN_Mona_DyT等高级注意力机制，实现自适应特征交互。
...
Mode --> |高级融合| HF["频域融合/注意力融合<br/>动态注意力机制<br/>自适应特征交互"]
Mode --> |动态注意力| DA["TransformerEncoderLayer_DAttention<br/>ASSA_SEFN_Mona_DyT<br/>自适应权重分配"]
EF --> End(["结束"])
MF --> End
LF --> End
HF --> End
...
- [rtdetr-r50-mm-mid-aifi-dattention.yaml:27](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-dattention.yaml#L27)
- [rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml:27](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml#L27)
...
- **新增** 高级融合机制：频域融合（FreqFusion）在P3层进行特征融合，保持相同尺度的安全性，提供更丰富的特征交互。
- **新增** 动态注意力机制：TransformerEncoderLayer_DAttention等模块实现自适应特征权重分配，增强跨模态信息融合效果。
...
subgraph "高级融合"
FF["频域融合<br/>FreqFusion"] --> B2
DA["动态注意力<br/>TransformerEncoderLayer_DAttention"] --> B2
ASSA["自适应融合<br/>ASSA_SEFN_Mona_DyT"] --> B2
end
```
...
- [rtdetr-r18-mm-early.yaml:12-49](file://ultralytics/cfg/models/rtmm/r18/rtdetr-r18-mm-early.yaml#L12-L49)
- [rtdetr-r50-mm-mid.yaml:11-66](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid.yaml#L11-L66)
- [aifi-dattention-CSP-MutilScaleEdgeInformationEnhance-ASF-P2-lite-BackboneFreqFusion.yaml:24](file://ultralytics/cfg/models/rtmm/r18/aifi-dattention-CSP-MutilScaleEdgeInformationEnhance-ASF-P2-lite-BackboneFreqFusion.yaml#L24)
- [rtdetr-r50-mm-mid-aifi-dattention.yaml:27](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-dattention.yaml#L27)
- [rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml:27](file://ultralytics/cfg/models/rtmm/r50/rtdetr-r50-mm-mid-aifi-assa-sefn-mona-dyt.yaml#L27)
...
- **新增** RTDETRMM配置集合：每个主干网络目录包含多个融合策略的配置文件，提供从基础到高级的渐进式选择，包括动态注意力机制配置。
...
### **新增** RTDETRMM高级融合架构详解
基于ResNet-18和ResNet-50的频域融合配置展示了先进的特征处理机制：

- **频域融合（FreqFusion）**：在P3层使用FreqFusion模块进行特征融合，保持相同尺度的安全性
- **动态注意力融合**：基于TransformerEncoderLayer_DAttention的自适应特征交互
- **自适应融合策略**：ASSA_SEFN_Mona_DyT等模块实现动态权重分配和特征增强
- **轻量化设计**：采用ASF-P2-lite策略，仅在关键层替换融合方式
- **模块化架构**：RGB和X分支独立处理，最后在P3层进行融合
- **参数配置**：包含详细的FreqFusion和动态注意力参数列表，支持多种融合策略
...
### **新增** 多模态路由器与配置解析器
RTDETRMM系统的核心组件，提供完整的多模态数据路由和配置管理能力：

- **MultiModalRouter**：通用RGB+X多模态数据路由器，支持零拷贝张量视图路由
- **MultiModalConfigParser**：YAML配置解析器，支持多模态架构的配置驱动数据流
- **HookManager**：钩子管理器，支持特征提取和批处理上下文管理
- **FeatureTap**：特征提取器，支持中间层特征访问
- **BatchContext**：批处理上下文，支持批量数据处理
...
- **新增** RTDETRMM独立架构：RTDETRMM已从传统RT-DETR中分离，提供独立的模型实现和配置体系。
- **新增** 多模态路由器：MultiModalRouter负责配置驱动的数据流路由，支持零拷贝张量操作。
...
R18_ADV["高级融合配置<br/>FreqFusion等"] --> TR
R50_ADV["动态注意力配置<br/>DAttention等"] --> TR
DATA["data(参考性质).yaml"] --> TR
MOD["data/multimodal"] --> TR
ENG["engine/multimodal"] --> TR
ROUTER["MultiModalRouter"] --> TR
PARSER["MultiModalConfigParser"] --> ROUTER
HOOK["HookManager"] --> ROUTER
```
...
- **新增** 高级融合技术：频域融合等技术在保持计算效率的同时提供更丰富的特征交互，适合复杂场景。
- **新增** 动态注意力机制：TransformerEncoderLayer_DAttention等模块实现自适应特征权重分配，提升跨模态信息融合效果。
- **新增** 多模态路由器优化：零拷贝张量视图路由减少内存复制开销，提高整体推理效率。
...
- **新增** RTDETRMM配置问题：确认使用正确的RTDETRMM配置文件，避免与传统RT-DETR配置混淆。
- **新增** 多模态路由器问题：检查MultiModalRouter初始化是否成功，确认配置解析器能够正确解析YAML配置。
- **新增** 动态注意力模块问题：验证TransformerEncoderLayer_DAttention等模块的正确安装和导入。

**章节来源**
- [多模态YAML构建指点.md:206-225](file://ultralytics/cfg/models/多模态YAML构建指点.md#L206-L225)

## 结论
通过统一的多模态YAML范式与完善的配置体系，本项目实现了YOLO与RT-DETR在RGB+X场景下的灵活配置与高效训练。RTDETRMM作为独立的模型家族，基于不同的主干网络提供了多样化的配置选择，从基础的早期融合到先进的频域融合技术和动态注意力机制，满足不同应用场景的需求。新增的多模态路由器和配置解析器进一步提升了系统的灵活性和性能。

遵循"早期/中期/晚期"融合策略与严格的分支书写规范，结合default.yaml提供的丰富训练与增强参数，可快速搭建并优化多模态目标检测系统。建议在实际工程中优先采用早期融合以获得更佳速度，再根据任务复杂度逐步过渡到中期/晚期融合，或采用高级融合技术如频域融合和动态注意力机制以获得更好的性能表现。

**新增** 多模态路由器的引入为系统提供了零拷贝张量路由和配置驱动的数据流管理能力，显著提升了多模态处理的效率和灵活性。动态注意力机制的应用进一步增强了跨模态特征整合能力，为复杂场景下的多模态目标检测提供了强有力的技术支撑。
...
- 命名与约定速查
  ...
  - 模型规模：n/s/m/l/x
- **新增** RTDETRMM配置集合
  - ResNet-18系列：基础配置，适合资源受限场景
  - ResNet-50系列：高性能配置，适合复杂场景，包含动态注意力机制
  - 融合策略：早期融合、中期融合、频域融合、动态注意力融合等多种选择
- **新增** 多模态路由器组件
  - MultiModalRouter：零拷贝张量路由
  - MultiModalConfigParser：配置解析
  - HookManager：钩子管理
  - FeatureTap：特征提取
  - BatchContext：批处理上下文
- 最小可用清单（Checklist）
  ...
  - **新增** [ ] 选择合适的RTDETRMM配置文件（基于所需主干网络和融合策略）
  - **新增** [ ] 验证多模态路由器初始化成功
  - **新增** [ ] 测试动态注意力模块功能正常
