...
## 更新摘要
**所做更改**
- 新增RTDETRMM模型配置集合的详细说明，包括基于ResNet-18和ResNet-50的配置目录结构
- 扩展融合架构配置内容，重点介绍频域融合（FreqFusion）等新型融合技术
...
**更新** 新增RTDETRMM模型配置集合的全面介绍，包括基于不同主干网络的配置变体和先进的融合架构。
...
I["频域融合示例<br/>aifi-dattention-CSP-MutilScaleEdgeInformationEnhance-ASF-P2-lite-BackboneFreqFusion.yaml"] --> C1
F --> J["多模态数据集配置<br/>data(参考性质).yaml"]
```
...
- **新增** RTDETRMM配置集合：基于不同主干网络（ResNet-18/50）的多样化配置变体，包含先进的融合架构如频域融合。
...
## 架构总览
多模态配置采用"配置驱动 + 第五字段路由"的统一范式，通过MultiModalRouter接管不同模态的输入与特征流，确保YOLO与RT-DETR均可复用同一套书写规则。RTDETRMM作为独立模型家族，基于不同主干网络提供多样化的融合策略，包括早期融合、中期融合和频域融合等先进架构。
...
END --> END2["ResNet-50系列<br/>rtdetr-r50-mm-*.yaml"]
end
subgraph "模型层"
YMM["YOLOMM<br/>yolo11n-mm-early.yaml"]
RMM18["RTDETRMM(r18)<br/>基于ResNet-18的配置"]
RMM50["RTDETRMM(r50)<br/>基于ResNet-50的配置"]
end
subgraph "数据与引擎"
...
- **新增** 高级融合技术：频域融合（FreqFusion）、注意力融合等，提供更精细的特征交互机制。
...
- **新增** RTDETRMM高级配置：基于ResNet-18和ResNet-50的不同主干网络，提供多样化的融合策略和性能选择。
...
Mode --> |高级融合| HF["频域融合/注意力融合<br/>精细化特征交互"]
EF --> End(["结束"])
MF --> End
LF --> End
HF --> End
...
- **新增** RTDETRMM配置集合：每个主干网络目录包含多个融合策略的配置文件，提供从基础到高级的渐进式选择。
...
## 结论
通过统一的多模态YAML范式与完善的配置体系，本项目实现了YOLO与RT-DETR在RGB+X场景下的灵活配置与高效训练。RTDETRMM作为独立的模型家族，基于不同的主干网络提供了多样化的配置选择，从基础的早期融合到先进的频域融合技术，满足不同应用场景的需求。遵循"早期/中期/晚期"融合策略与严格的分支书写规范，结合default.yaml提供的丰富训练与增强参数，可快速搭建并优化多模态目标检测系统。建议在实际工程中优先采用早期融合以获得更佳速度，再根据任务复杂度逐步过渡到中期/晚期融合，或采用高级融合技术如频域融合以获得更好的性能表现。
...
- **新增** RTDETRMM配置集合
  ...
  - ResNet-50系列：高性能配置，适合复杂场景
  - 融合策略：早期融合、中期融合、频域融合等多种选择
- 最小可用清单（Checklist）
