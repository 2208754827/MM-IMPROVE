...
class YOLOMultiModalImageDataset(YOLODataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Log multimodal setup in ASCII to avoid terminal encoding issues.
        LOGGER.info(f"MultiModal: initialized RGB+{self.x_modality} dataset")
        LOGGER.info(f"MultiModal: samples={self.ni}, x_modality_dir='{self.x_modality_dir}'")
        if self.enable_self_modal_generation:
            LOGGER.info("MultiModal: self-modal generation enabled")

    def _validate_multimodal_data(self):
        """Validate multimodal data integrity and consistency."""
        if not hasattr(self, 'im_files') or not self.im_files:
            LOGGER.warning("MultiModal: RGB image list is empty, skip integrity check")
            return
        ...
        if not x_dir.exists():
            if self.enable_self_modal_generation:
                LOGGER.info(f"MultiModal: X directory not found: {x_dir}, using self-modal generation")
            else:
                LOGGER.debug(f"MultiModal: X directory not found: {x_dir}, will try fallback paths")
        ...
        LOGGER.info(f"MultiModal: integrity check {valid_ratio:.1%} ({valid_count}/{sample_size})")

        if valid_ratio < 0.5 and not self.enable_self_modal_generation:
            LOGGER.warning("MultiModal: low integrity ratio, enable self-modal generation or check data paths")
    ...
    def get_valid_indices(self):
        ...
        if not hasattr(self, '_valid_indices_cache'):
            ...
            try:
                ...
                log_enabled = True
            # Keep terminal progress bars clean by default (prevents tqdm line corruption in Windows/PyCharm terminal).
            if str(os.getenv("YOLO_MM_QUIET_PROGRESS", "true")).lower() == "true":
                log_enabled = False

            if log_enabled:
                LOGGER.info("MultiModal: scanning valid multimodal indices...")
            ...
            if log_enabled:
                LOGGER.info(f"MultiModal: found {valid_count}/{self.ni} ({valid_ratio:.1%}) valid samples")

                if valid_count == 0:
                    LOGGER.error("MultiModal: no valid samples found, check data paths and pairing config")
                elif valid_ratio < 0.8:
                    LOGGER.warning(f"MultiModal: valid sample ratio is low ({valid_ratio:.1%}), check data integrity")
