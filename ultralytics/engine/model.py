...
class Model(torch.nn.Module):
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        Raises:
            ValueError: 当 rgb_source 或 x_source 缺失时
        """
        # 参数验证（支持单模态推理）
        if rgb_source is None and x_source is None:
            raise ValueError(
                "至少需要提供 rgb_source 或 x_source 参数之一。\n"
                "示例:\n"
        ...
        # 传递显式参数给 predictor
        if is_cli:
            return self.predictor.predict_cli(rgb_source=rgb_source, x_source=x_source)
        else:
            return self.predictor(rgb_source=rgb_source, x_source=x_source, stream=stream)
