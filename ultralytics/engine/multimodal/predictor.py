...
class MultiModalPredictor:
    ...
    def __call__(
        ...
        **kwargs
        ...
        sample_specs = resolver.resolve(rgb_source=rgb_source, x_source=x_source)
        ...
        # 流式推理
        if stream:
            return self._stream_inference(dataset, save=save, save_txt=save_txt, save_dir=save_dir)
        else:
            return list(self._stream_inference(dataset, save=save, save_txt=save_txt, save_dir=save_dir))
    ...
    def _stream_inference(
        ...
        for sample in dataset:
            ...
            if self.is_rtdetr:
                ...
                pred = self._postprocess_rtdetr(preds_tensor, sample)
            else:
    ...
    def _postprocess_rtdetr(self, preds: torch.Tensor, sample: Dict) -> torch.Tensor:
        ...
        if self.debug:
            LOGGER.info(f"[DEBUG RTDETR] 置信度阈值: {self.conf}")
        ...
        if len(det) > self.max_det:
            ...
            if self.debug:
                LOGGER.info(f"[DEBUG RTDETR] 限制到max_det={self.max_det}")
