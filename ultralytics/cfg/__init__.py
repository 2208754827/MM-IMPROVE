...
def cfg2dict(cfg: Union[str, Path, Dict, SimpleNamespace]) -> Dict:
    ...
    Notes:
        ...
        - If cfg is None, returns an empty dictionary.
    """
    if cfg is None:
        return {}  # 处理None情况，返回空字典
...
def get_cfg(cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None) -> SimpleNamespace:
    ...
    # 如果cfg为空（None转换后）或者没有提供cfg，使用默认配置
    if not cfg:
    ...
    if overrides:
        ...
        # Allow `scale=n/s/m/l/x` as an alias for explicitly selecting YAML `scales` (model compound scaling).
        # `scale` is also a float hyperparameter for augmentation. When used as a string here, normalize it into
        # `model_scale` and preserve the augmentation `scale` value from the base cfg.
        if isinstance(overrides.get("scale"), str):
            _ms = overrides["scale"].strip().lower()
            if _ms in {"n", "s", "m", "l", "x"}:
                if overrides.get("model_scale") not in (None, _ms):
                    raise ValueError("Both 'scale' (as model scale alias) and 'model_scale' are set. Please keep only one.")
                overrides["model_scale"] = _ms
                overrides.pop("scale", None)

        check_dict_alignment(cfg, overrides)
        cfg = {**cfg, **overrides}  # merge cfg and overrides dicts (prefer overrides)

    # Normalize base cfg in case `scale` was set as a model-scale alias in a YAML config file.
    if isinstance(cfg.get("scale"), str):
        _ms = cfg["scale"].strip().lower()
        if _ms in {"n", "s", "m", "l", "x"}:
            if cfg.get("model_scale") not in (None, _ms):
                raise ValueError("Both 'scale' (as model scale alias) and 'model_scale' are set. Please keep only one.")
            cfg["model_scale"] = _ms
            cfg["scale"] = DEFAULT_CFG_DICT.get("scale", 0.5)
        else:
            raise TypeError(
                f"'scale={cfg['scale']}' is invalid. Use a float (augmentation) or one of n/s/m/l/x (model scale)."
            )

    # Optional explicit augmentation scale override. Useful when `scale` is used for model scale alias.
    if cfg.get("img_scale") is not None:
        cfg["scale"] = cfg["img_scale"]
...
def entrypoint(debug: str = "") -> None:
    ...
    if "rtdetr" in stem:  # guess architecture
        ...
        model = SAM(model)
    elif "-mm" in stem:  # YOLOMM multi-modal model
        try:
            from ultralytics.models.yolo.model import YOLOMM
            model = YOLOMM(model, task=task)
        except ImportError as e:
            LOGGER.warning(f"YOLOMM multi-modal components not available ({e}), using standard YOLO")
