...
class FCMFeatureFusion(nn.Module):
    ...
    def forward(self, x1, x2=None):
        ...
        if self.fcm is not None:
            p = next(self.fcm.parameters(), None)
            if p is not None and p.device != x1.device:
                self.to(device=x1.device)
            # If autocast is off and dtype mismatches, align inputs to module dtype.
            if p is not None and x1.dtype != p.dtype and not torch.is_autocast_enabled():
                x1 = x1.to(p.dtype)
                x2 = x2.to(p.dtype)
