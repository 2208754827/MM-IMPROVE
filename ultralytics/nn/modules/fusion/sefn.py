...
class SEFN(nn.Module):
    def __init__(self, dim: int | None = None, ffn_expansion: float = 2.0, bias: bool = True) -> None:
        super().__init__()
        self.dim = dim
        ...
        # If parser already provides dim (usual in YAML), build once here so model.to()/amp can manage dtype/device.
        if self.dim is not None:
            self._build_if_needed(int(self.dim))
    ...
    def forward(self, x1, x2=None):
        ...
        # Keep parameters in fp32 for AMP stability, only align device.
        if self.project_in is not None and self.project_in.weight.device != x1.device:
            self.to(device=x1.device)
        # If autocast is not active and dtype mismatches, align inputs to module dtype.
        if self.project_in is not None and x1.dtype != self.project_in.weight.dtype and not torch.is_autocast_enabled():
            x1 = x1.to(self.project_in.weight.dtype)
            x2 = x2.to(self.project_in.weight.dtype)
