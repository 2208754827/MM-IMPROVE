...
class ScalarGate(nn.Module):
    ...
    def __init__(self, channels: int):
        ...
        # 单路 SE 分支：全局标量门控，用于单路 Concat 后特征的通道精炼
        self.se_fc = nn.Sequential(
            nn.Conv2d(channels, 1, kernel_size=1, stride=1, padding=0, bias=True)
        )

    def forward(self, x) -> torch.Tensor:
        # 单路张量输入：全局标量 SE 通道精炼模式
        if isinstance(x, torch.Tensor):
            g = F.adaptive_avg_pool2d(x, output_size=1)  # (B, C, 1, 1)
            z = torch.sigmoid(self.se_fc(g))              # (B, 1, 1, 1)
            return x * z
        # 双路列表/元组输入：模态融合门控模式
        if not isinstance(x, (list, tuple)) or len(x) != 2:
            raise TypeError("ScalarGate 期望单路张量或 [rgb, ir] 双路列表，实际得到: {}".format(type(x)))
        rgb, ir = x
        if not (isinstance(rgb, torch.Tensor) and isinstance(ir, torch.Tensor)):
            raise TypeError("ScalarGate 的双路输入必须为张量")
        if rgb.shape != ir.shape:
            raise ValueError(f"两路输入形状需一致，got {rgb.shape} vs {ir.shape}")
...
class ChannelGate(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        # 双路列表/元组输入：模态融合门控模式
        if not isinstance(x, (list, tuple)) or len(x) != 2:
            raise TypeError("ChannelGate 期望单路张量或 [rgb, ir] 双路列表，实际得到: {}".format(type(x)))
        rgb, ir = x
        if not (isinstance(rgb, torch.Tensor) and isinstance(ir, torch.Tensor)):
            raise TypeError("ChannelGate 的双路输入必须为张量")
        if rgb.shape != ir.shape:
            raise ValueError(f"两路输入形状需一致，got {rgb.shape} vs {ir.shape}")
        gap_rgb = F.adaptive_avg_pool2d(rgb, output_size=1)
        gap_ir = F.adaptive_avg_pool2d(ir, output_size=1)
...
class SEChannelAttention(nn.Module):
    """
    【核心创新】
    SEChannelAttention 实现了面向融合后特征的单路 SE（Squeeze-and-Excitation）通道注意力优化机制。
    与 ChannelGate/ScalarGate 不同，本模块接收单路张量输入（如 Concat 后的融合特征），
    通过 SE 结构对各通道的重要性独立建模，抑制冗余通道、增强语义显著通道，
    从而提升融合后特征的判别能力。

    【解决的问题】
    - Concat 融合后特征各通道重要性不均等，存在大量冗余通道
    - 直接下游卷积对所有通道等权对待，缺乏通道级自适应调节
    - 轻量级通道精炼在检测任务中有助于提升小目标响应

    【工作机制】
    输入特征 x (B, C, H, W):
    1. GAP: 全局平均池化 -> (B, C, 1, 1)
    2. FC1: 1x1卷积压缩通道 C -> C//reduction，ReLU 激活
    3. FC2: 1x1卷积恢复通道 C//reduction -> C，Sigmoid 激活得到权重 w (B, C, 1, 1)
    4. 输出: x * w（逐通道加权，输入输出 shape 完全一致）

    【YAML 使用方式】
    - [[4, 9], 1, Concat, [1]]          # 10  Concat RGB+IR -> (B, 2C, H, W)
    - [-1, 1, SEChannelAttention, []]   # 11  通道注意力精炼，自动推断 2C
    - [-1, 1, Conv, [256, 1, 1]]        # 12  投影到目标通道数

    【通道数说明】
    __init__(channels) 中 channels 即上一层的输出通道数（由 tasks.py 自动注入，无需手动指定）。
    Concat 后通道数为 2C（如 256+256=512），模块自适应处理，输出通道数不变。
    """

    def __init__(self, channels: int, reduction: int = 16):
        """
        Args:
            channels (int): 输入特征的通道数 C（由 tasks.py 自动从上游推断注入）。
            reduction (int): 通道压缩比，默认 16。压缩后隐层维度为 max(1, C // reduction)。
        """
        super().__init__()
        hidden = max(1, channels // reduction)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),                                  # (B, C, 1, 1)
            nn.Conv2d(channels, hidden, kernel_size=1, bias=True),   # 压缩
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden, channels, kernel_size=1, bias=True),   # 还原
            nn.Sigmoid(),                                             # 通道权重 in [0, 1]
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): 单路输入特征，shape (B, C, H, W)。
        Returns:
            torch.Tensor: 通道加权后的特征，shape 与输入完全一致 (B, C, H, W)。
        """
        if not isinstance(x, torch.Tensor):
            raise TypeError(
                f"SEChannelAttention 期望单路张量输入，got {type(x)}。"
                "若需要双路融合请使用 ChannelGate。"
            )
        return x * self.se(x)
