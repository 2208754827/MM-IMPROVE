...
__all__ = (
    ...
    "SCDown",
    "TorchVision",
    # RTDETR ResNet modules
    "get_activation",
    "ConvNormLayer",
    "BasicBlock",
    "RTDETRBottleNeck",
    "Blocks",
    # Area-attention modules
    "ABlock",
    "A2C2f",
)
...
class RGCSPELAN(nn.Module):
    """Rep Ghost CSP-ELAN block."""

    def __init__(self, c1: int, c2: int, n: int = 1, scale: float = 0.5, e: float = 0.5):
        super().__init__()
        if n < 1:
            raise ValueError(f"RGCSPELAN requires n >= 1, got n={n}")

        self.c = max(int(c2 * e), 1)
        self.mid = max(int(self.c * scale), 1)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
...
################################### RTDETR ResNet Support Functions ###################################
...
class RTDETRBottleNeck(nn.Module):
    expansion = 4
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        self.shortcut = shortcut
        # 当输入输出通道数不匹配时，需要shortcut连接
        if not shortcut or ch_in != ch_out * self.expansion:
            if variant == 'd' and stride == 2:
                self.short = nn.Sequential(OrderedDict([
                    ('pool', nn.AvgPool2d(2, 2, 0, ceil_mode=True)),
            ...
            self.shortcut = False  # 需要通过short连接
