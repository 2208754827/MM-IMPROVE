#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""FPGM structured iterative pruning for RTDETRMM VIF models.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="FPGM iterative pruning for RTDETRMM VIF models")
    parser.add_argument("--weights", type=str, default=str(default_weights), help="Input .pt checkpoint")
    parser.add_argument("--output", type=str, default="prune_outputs_fpgm_only", help="Output .pt path or directory")
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_fpgm_r{ratio_str}_{timestamp}.pt"
...
def build_meta_pruner(wrapper: ModelWrapper, tp, args, vis, ir):
    importance = tp.importance.FPGMImportance(p=2, group_reduction="mean", normalizer="mean")
...
def fpgm_scores_for_root(pruner, spec: RootSpec) -> torch.Tensor:
    tp = tp_global()
    channels = pruner.DG.get_out_channels(spec.module)
    if channels is None or channels <= 0:
        raise RuntimeError("Invalid output channels for FPGM scoring.")
    group = pruner.DG.get_pruning_group(spec.module, spec.pruning_fn, idxs=list(range(channels)))
    scores = tp.importance.FPGMImportance(p=2, group_reduction="mean", normalizer="mean")(group)
    if scores is None:
        raise RuntimeError("FPGM importance returned None.")
    ...
    while True:
        ...
        try:
            ...
            for spec in current_specs:
                ...
                try:
                    scores = fpgm_scores_for_root(pruner, spec)
            ...
            if best_candidate is None:
                log(f"Round {round_idx}: no more safe FPGM pruning candidates.")
                break

            raw_score, spec, idxs = best_candidate
            log(
                f"Round {round_idx}: prune {spec.name} "
                f"(layer={spec.layer_idx}, n_prune={len(idxs)}, fpgm={raw_score:.4e})"
            )
            try:
...
def main() -> int:
    ...
    print("Note   : this script uses DepGraph + FPGM importance only, without extra FLOPs-biased ranking.")
