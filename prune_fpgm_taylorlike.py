#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""FPGM-TaylorLike structured iterative pruning for RTDETRMM RGB/IR fusion models.
...
- Uses torch-pruning MetaPruner/DepGraph with FPGM importance for root pruning.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="FPGM-TaylorLike iterative pruning for RTDETRMM VIF models")
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_fpgm_taylorlike_r{ratio_str}_{timestamp}.pt"
...
def sanitize_checkpoint_for_save(orig_ckpt: dict | None, model: nn.Module) -> dict:
    ...
    ckpt["prune_method"] = "fpgm_taylorlike"
    return ckpt
    ...
    while True:
        ...
        try:
            ...
            for spec in current_specs:
                ...
                try:
                    scores = fpgm_scores_for_conv_out_channels(spec.module)
