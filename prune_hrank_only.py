#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""HRank-style structured iterative pruning for RTDETRMM VIF models.
...
- HRank-style feature map rank importance
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="HRank iterative pruning for RTDETRMM VIF models")
    parser.add_argument("--weights", type=str, default=str(default_weights), help="Input .pt checkpoint")
    parser.add_argument("--output", type=str, default="prune_outputs_hrank_only", help="Output .pt path or directory")
    ...
    parser.add_argument("--rank-samples", type=int, default=1, help="Number of forward passes used to average HRank scores.")
    parser.add_argument(
        "--rank-resolution",
        type=int,
        default=32,
        help="Adaptive pooling size used before matrix-rank estimation. <=0 disables pooling.",
    )
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_hrank_r{ratio_str}_{timestamp}.pt"
...
def hrank_scores_for_feature_map(output: torch.Tensor, rank_resolution: int) -> torch.Tensor:
    if not isinstance(output, torch.Tensor) or output.ndim != 4:
        raise RuntimeError("HRank requires 4D convolution outputs.")

    x = output.detach().float()
    if rank_resolution > 0 and (x.shape[-2] != rank_resolution or x.shape[-1] != rank_resolution):
        x = F.adaptive_avg_pool2d(x, (rank_resolution, rank_resolution))

    b, c, h, w = x.shape
    maps = x.reshape(b * c, h, w)
    ranks = torch.linalg.matrix_rank(maps).to(dtype=torch.float32, device=x.device)
    return ranks.view(b, c).mean(dim=0)


def collect_hrank_scores(
    model: nn.Module,
    specs: list[RootSpec],
    batch_size: int,
    imgsz: int,
    ...
    score_sum: dict[str, torch.Tensor] = {}
    call_count: dict[str, int] = {spec.name: 0 for spec in specs}
    hooks = []

    def make_hook(name: str):
        def _hook(_module, _inputs, output):
            scores = hrank_scores_for_feature_map(output, rank_resolution)
            if name in score_sum:
                score_sum[name] = score_sum[name] + scores
            else:
                score_sum[name] = scores.clone()
            call_count[name] += 1

        return _hook

    for spec in specs:
        hooks.append(spec.module.register_forward_hook(make_hook(spec.name)))

    wrapper = ModelWrapper(model)
    try:
        wrapper.eval()
        with torch.no_grad():
            for _ in range(max(1, rank_samples)):
    ...
    averaged = {}
    for name, scores in score_sum.items():
        denom = max(call_count.get(name, 1), 1)
        averaged[name] = scores / float(denom)
    return averaged
...
def prune_one_round(
    ...
    while True:
        ...
        try:
            ...
            hrank_scores_map = collect_hrank_scores(
                model=model,
                specs=current_specs,
                ...
                rank_resolution=args.rank_resolution,
            )
            ...
            for spec in current_specs:
                ...
                try:
                    scores = hrank_scores_map.get(spec.name)
                    if scores is None:
                        raise RuntimeError("Missing HRank scores for current root.")
            ...
            if best_candidate is None:
                log(f"Round {round_idx}: no more safe HRank pruning candidates.")
                break

            raw_score, spec, idxs = best_candidate
            log(
                f"Round {round_idx}: prune {spec.name} "
                f"(layer={spec.layer_idx}, n_prune={len(idxs)}, hrank={raw_score:.4e})"
            )
            try:
...
def main() -> int:
    ...
    if args.rank_samples < 1:
        raise ValueError("--rank-samples must be >= 1")
    ...
    print("Note   : this script uses DepGraph + HRank-style feature map rank scoring, without extra FLOPs-biased ranking.")
