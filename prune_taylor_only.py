...
- Taylor first-order importance only
- iterative channel pruning with fresh gradients every round
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Taylor-only iterative pruning for RTDETRMM VIF models")
    parser.add_argument("--weights", type=str, default=str(default_weights), help="Input .pt checkpoint")
    parser.add_argument("--output", type=str, default="prune_outputs_taylor_only", help="Output .pt path or directory")
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_taylor_r{ratio_str}_{timestamp}.pt"
...
def taylor_scores_for_conv_out_channels(conv: nn.Conv2d) -> torch.Tensor:
    if conv.weight.grad is None:
        raise RuntimeError("Missing gradients for Taylor scoring.")
    ...
    while True:
        ...
        try:
            ...
            log(
                ...
                f"(layer={spec.layer_idx}, n_prune={len(idxs)}, taylor={raw_score:.4e})"
            )
            try:
...
def main() -> int:
    ...
    print("Note   : this script uses DepGraph + Taylor only, without extra FLOPs-biased ranking.")
