...
This script targets the RTDETRMM variant with:
- RGB branch: backbone layers 0-4
- IR branch: backbone layers 5-9
- Fusion layer: backbone layer 10 (PIAFusionBlock)
- Compression layer: backbone layer 11 (Conv 512 -> 256)
- Head modules containing DAttention and RTDETRDecoder

Key design points:
- Uses torch-pruning with Taylor importance.
- Wraps the original model as forward(vis, ir) while internally feeding 6-channel input.
- Preserves the PIAFusionBlock branch alignment by skipping the fusion input roots and pruning structurally safe
  backbone outputs, including Conv layers and CSP/C2f-style block output convolutions.
...
def parse_args() -> argparse.Namespace:
    pia_default = Path("ReTest") / "aifi-dattention-CSP-MutilScaleEdgeInformationEnhance-ASF-P2-lite-PIAFusionBlock" / "weights" / "best.pt"
