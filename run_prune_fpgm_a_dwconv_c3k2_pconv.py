#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""FPGM pruning entry for A-DWConv-C3k2_PConv aligned to the Taylor setup.

This wrapper keeps the same pruning scope and runtime settings as the current
Taylor runner, while swapping only the importance score to FPGM.

Usage:
    python run_prune_fpgm_a_dwconv_c3k2_pconv.py
"""
...
OUTPUT = ROOT / "prune_outputs_fpgm_compare_c3k2"
...
ROUND_TO = 8
INCLUDE_NECK = True
LAYER_END = 39  # Exclusive; matches the Taylor entry's open pruning scope.
FLOPS_BIAS = 1.25
...
def main() -> int:
    ...
    import prune_fpgm_taylorlike

    sys.argv = [
        "prune_fpgm_taylorlike.py",
        "--weights", str(WEIGHTS),
        "--output", str(OUTPUT),
        "--data", str(DATA),
        "--device", DEVICE,
    ...
    return prune_fpgm_taylorlike.main()
