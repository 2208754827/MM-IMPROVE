#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""BN-Slimming style structured iterative pruning for RTDETRMM VIF models.

This script is independent from the downloaded pruning scripts.
It uses:
- torch-pruning MetaPruner + DependencyGraph
- BN gamma magnitude as channel importance
- iterative structured channel pruning

Note:
- Classical Network Slimming also adds L1 regularization on BN scales during training.
- This script focuses on the pruning stage only, which is the usual baseline form
  when comparing different post-training structured pruning rules in one codebase.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="BN-Slimming iterative pruning for RTDETRMM VIF models")
    parser.add_argument("--weights", type=str, default=str(default_weights), help="Input .pt checkpoint")
    parser.add_argument("--output", type=str, default="prune_outputs_bn_slimming", help="Output .pt path or directory")
...
def make_output_path(output_arg: str, ratio: float) -> Path:
    ...
    return out / f"pruned_bnslim_r{ratio_str}_{timestamp}.pt"
...
def sanitize_checkpoint_for_save(orig_ckpt: dict | None, model: nn.Module) -> dict:
    ...
    ckpt["prune_method"] = "bn_slimming_only"
    return ckpt
...
class RootSpec:
    ...
    layer_idx: int
    module: nn.Module
    bn_module: nn.modules.batchnorm._BatchNorm
    pruning_fn: Callable
...
def resolve_bn_for_submodule(layer: nn.Module, sub_name: str):
    if sub_name == "conv":
        parent = layer
    else:
        parent_name = sub_name.rsplit(".", 1)[0]
        try:
...
def collect_root_specs(model: nn.Module, pruner, layer_end: int, include_neck: bool) -> list[RootSpec]:
    ...
    for idx in range(min(layer_end, len(model.model))):
        ...
        for sub_name, module in layer.named_modules():
            ...
            if module.groups != 1:
                continue
            bn_module = resolve_bn_for_submodule(layer, sub_name)
            if bn_module is None:
                continue
            if bn_module.num_features != module.out_channels:
                continue
...
def bn_scale_scores_for_conv_out_channels(spec: RootSpec) -> torch.Tensor:
    bn = spec.bn_module
    if bn.weight is None:
        raise RuntimeError("BN affine weight is missing.")
    return bn.weight.detach().abs().to(spec.module.weight.device)
    ...
    while True:
        ...
        try:
            ...
            for spec in current_specs:
                ...
                try:
                    scores = bn_scale_scores_for_conv_out_channels(spec)
            ...
            if best_candidate is None:
                log(f"Round {round_idx}: no more safe BN-Slimming pruning candidates.")
                break

            raw_score, spec, idxs = best_candidate
            log(
                f"Round {round_idx}: prune {spec.name} "
                f"(layer={spec.layer_idx}, n_prune={len(idxs)}, bn_gamma={raw_score:.4e})"
            )
            try:
...
def main() -> int:
    ...
    print("Note   : this script uses DepGraph + BN gamma magnitude. Full classical Network Slimming also adds BN sparsity regularization during training.")
